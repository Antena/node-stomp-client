...
function StompClient(opts) {
  ...
  var address, port, user, pass, protocolVersion, vhost, reconnectOpts, tlsOpts, heartbeatTimeout, heartbeatInterval;
  ...
  if(arguments.length !== 1 || typeof opts === 'string') {
    ...
    heartbeatTimeout = arguments[8];
    heartbeatInterval = arguments[9];
    tlsOpts = arguments[7];
    if(tlsOpts === true) {
      tlsOpts = {};
    }
  ...
  this.heartbeatInterval = heartbeatInterval || 0;
...
StompClient.prototype.onConnect = function() {
  ...
  if(this.heartbeatTimeout && this.heartbeatTimeout > 0) {
    headers['heart-beat'] = this.heartbeatTimeout + ',0';

    if (this.heartbeatInterval && this.heartbeatInterval > 0) {
      this.heartbeatIntervalId = setInterval(() => {
        new StompFrame().send(this.stream);
